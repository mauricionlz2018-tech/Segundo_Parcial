...
async function sendViaBrevo(to: string, subject: string, html: string) {
  ...
  if (!apiKey || !fromEmail) {
    throw new Error("Faltan variables de entorno BREVO_API_KEY o BREVO_SMTP_USER")
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(`Error al enviar correo (Brevo ${response.status}): ${errorText}`)
  }

  const data = await response.json()
  console.log(`[sendEmail] Enviado: ${data.messageId}`)
  return data
}
...
export async function sendPasswordResetEmail(
  ...
  await sendViaBrevo(email, "Código de recuperación de contraseña - UES", html)
  console.log(`[sendPasswordResetEmail] Enviado a: ${email}`)
  return true
}
...
export async function sendWelcomeEmail(
  ...
  await sendViaBrevo(email, "Bienvenido a UES - Tus credenciales de acceso", html)
  console.log(`[sendWelcomeEmail] Enviado a: ${email}`)
  return true
}
...
export async function sendUpcomingSessionAlert(
  ...
  await sendViaBrevo(email, `Recordatorio: ${sessionName}`, html)
  console.log(`[sendUpcomingSessionAlert] Enviado a: ${email}`)
  return true
}